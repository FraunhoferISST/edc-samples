...
public class CatalogNodeDirectory implements TargetNodeDirectory {
    ...
    public List<TargetNode> readNodesFromJson() {
        try {
            List<TargetNode> nodes = objectMapper.readValue(new File("fc/fc-embedded-old/refresh-catalog/catalog-node-directory.json"), new TypeReference<List<TargetNode>>(){});
            return nodes;
        } catch (IOException e) {
